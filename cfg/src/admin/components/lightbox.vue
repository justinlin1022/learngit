...
export default {
  ...
  mounted() {
    ...
    if(!this.imageList.length){
      this.imageList = [{src:'',thumb:'',caption:''}];
    }
  ...
  watch: {
    ...
    select() {
      ...
      if (this.select <= halfDown) {
        ...
        this.$set(this, 'displayThumbs', this.imageList.slice(0, this.nThumbs))
        return
      }
      ...
      if (this.select >= this.countImages - halfDown) {
        ...
        this.$set(this, 'displayThumbs', this.imageList.slice(-this.nThumbs))
        return
      }

      this.$set(this, 'beginThumbIndex', this.select - halfDown + mod)
      this.$set(this, 'thumbSelect', halfDown - mod)
      this.$set(this, 'displayThumbs', this.imageList.slice(this.select - halfDown + mod, this.select + halfDown + 1))
